var app = angular.module('sprtsPollApp', []);
		
		app.controller('sprtsPollCtrl', function($scope, $http) {
			
			$scope.homeTeamWinCount = 0;
			$scope.drawCount = 0;
			$scope.awayTeamWinCount = 0;
			
			$scope.calPollCount = function() {
				var pollOption = $scope.formData.pollOption;
				if(angular.equals(pollOption,'homeTeamWin')){
					$scope.homeTeamWinCount++;
				}else if(angular.equals(pollOption,'draw')){
					$scope.drawCount++;
				}else if(angular.equals(pollOption,'awayTeamWin')){
					$scope.awayTeamWinCount++;
				}
			}
			...
			$scope.allTeams=[
						...
						$scope.previousDisplayedCategory = localStorage.getItem("category");
						
						function getRandomTeam(previousDisplayedCategory){
							$scope.tempRandomTeam =$scope.allTeams[Math.floor(Math.random() * $scope.allTeams.length)];
							if(!angular.equals($scope.tempRandomTeam.sport,previousDisplayedCategory)){
							$scope.randomTeam = $scope.tempRandomTeam;
								localStorage.setItem("category", JSON.stringify($scope.tempRandomTeam.sport));
								return $scope.tempRandomTeam;
							}else {
								return getRandomTeam(previousDisplayedCategory);
							}
						}
						getRandomTeam($scope.previousDisplayedCategory);								
